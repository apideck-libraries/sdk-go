...
func newCustomMappings(rootSDK *Apideck, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *CustomMappings {
...
func (s *CustomMappings) List(ctx context.Context, unifiedAPI string, serviceID string, consumerID *string, appID *string, opts ...operations.Option) (*operations.VaultCustomMappingsAllResponse, error) {
	...
	globals := operations.VaultCustomMappingsAllGlobals{
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/vault/custom-mappings/{unified_api}/{service_id}", request, globals)
	if err != nil {
	...
	res := &operations.VaultCustomMappingsAllResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.GetCustomMappingsResponse
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.GetCustomMappingsResponse = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
