...
type ConnectionConsents struct {
	rootSDK          *Apideck
	sdkConfiguration config.SDKConfiguration
	hooks            *hooks.Hooks
}

func newConnectionConsents(rootSDK *Apideck, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *ConnectionConsents {
...
func (s *ConnectionConsents) List(ctx context.Context, serviceID string, unifiedAPI string, consumerID *string, appID *string, opts ...operations.Option) (*operations.VaultConnectionConsentsAllResponse, error) {
	...
	globals := operations.VaultConnectionConsentsAllGlobals{
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/vault/connections/{unified_api}/{service_id}/consent", request, globals)
	if err != nil {
	...
	res := &operations.VaultConnectionConsentsAllResponse{
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out components.GetConsentRecordsResponse
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.GetConsentRecordsResponse = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
